/* ══════════════════════════════════════════════════════
   ...
   is stored as a SHA-256 hash in Upstash Redis — never in
   plaintext, and never in this codebase.

   Body: { "currentPassword": "...", "newPassword": "..." }
   Response: { ok: true } or { ok: false, error }

   Requires Upstash Redis to be connected (see SETUP_GUIDE.txt) —
...
module.exports = async (req, res) => {
  ...
  if (!isConfigured()) {
    res.status(200).json({
      ok: false,
      error: 'Connect Upstash Redis first (see SETUP_GUIDE.txt) — there is nowhere to save a new password yet.'
    });
    return;
  }
