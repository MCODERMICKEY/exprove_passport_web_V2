...
function handleServiceForm(formElement, serviceName, amountGHS) {
  ...
  if (!email) {
    // Email is optional on some forms (e.g. birth certificate informant),
    // but Paystack still needs *a* valid-format address to process the
    // payment. Fall back to one built from the phone number — the
    // applicant just won't get an emailed Paystack receipt.
    const digits = (data.phone || 'guest').replace(/\D/g, '') || 'guest';
    email = `${digits}@no-email.exprove.local`;
  }
...
/* ══════════════════════════════════════════════════════
   FILE UPLOADS — Client Photo / Ghana Card / Birth Certificate
   Files are shown as an instant local preview AND uploaded for
   real to Vercel Blob storage via /api/upload. The resulting
   public URL is written into a hidden form field, so it travels
   along with the rest of the applicant's data when the form is
   submitted — the admin can then open the file straight from
   the dashboard.

   If Blob storage isn't connected yet (see SETUP_GUIDE.txt),
   the upload silently fails and a status note tells the
   applicant to send the file via WhatsApp/email instead — the
   rest of the form still submits normally either way.
══════════════════════════════════════════════════════ */
const MAX_UPLOAD_BYTES = 3 * 1024 * 1024; // 3MB — keeps the base64 payload safely under Vercel's request size limit
...
function setUploadStatus(statusId, state, message) {
  if (!statusId) return;
...
function bindPhotoPreview(inputId, circleId, hiddenId, statusId) {
  ...
  input.addEventListener('change', async () => {
    ...
    if (file.size > MAX_UPLOAD_BYTES) {
      showNotification('Photo is larger than 3MB — please choose a smaller file.', 'error');
      input.value = '';
      return;
    }
    ...
    setUploadStatus(statusId, 'uploading', 'Uploading…');

    try {
      const url = await uploadFileToBlob(file);
      if (hidden) hidden.value = url;
      setUploadStatus(statusId, 'success', '✓ Uploaded — attached to your application');
    } catch (err) {
      setUploadStatus(statusId, 'failed', "⚠ Couldn't upload — please also send this via WhatsApp or email");
    }
  });
}

/* ── Generic document upload (Ghana Card / Birth Certificate) ──
   Works for images (shows a thumbnail) and PDFs/other files
   (shows a filename chip) — and, like the photo above, uploads
   for real to Vercel Blob storage. */
function bindFilePreview(inputId, previewId, hiddenId, statusId) {
  const input = document.getElementById(inputId);
  ...
  input.addEventListener('change', async () => {
    ...
    if (!file) return;
    if (file.size > MAX_UPLOAD_BYTES) {
      showNotification('File is larger than 3MB — please choose a smaller file.', 'error');
      input.value = '';
      return;
    }
